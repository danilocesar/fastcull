...
- **Dependencies compile optimised even in debug.** (user decision
  ...
  `target/` is cleaned by the Manager at the END of every unit (`cargo
  clean -p` of the three workspace crates, CLAUDE.md M9); never clean it
  mid-unit, and never run a full `cargo clean`.
